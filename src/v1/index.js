...
module.exports = {
  newMark: new CommonQuery({
    query: (ctx) => {
      ...
      return {
        text: 'INSERT INTO mark (title, description, items_id, markers,\
          berateds, create_timestamp) VALUES ($1, $2, $3, $4, $5, $6)\
          RETURNING mark_id AS markId;',
        values: [title, description, itemsId, markers, berateds,
          createTimestamp]
      };
    },
    fn: (ctx, res) => {
      if (res.rowCount > 0) {
        return ctx.body = '{"state": "1101", \
  ...
  newMarkItem: new CommonQuery({
    query: (ctx) => {
      ...
      return {
        text: 'INSERT INTO mark_item (description, total_value)\
                              VALUES ($1, $2);',
